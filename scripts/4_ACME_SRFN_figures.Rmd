...
Adding two other combined plots for presentations emphasizing species with strong relationships with linear features and polygonal features

## Linear features combined plot


First re-do plots above to add color to highlight the linear features odds ratios 
...
# List of plots
lf_plots <- list (grey_wolf_odds_plot2,
                  coyote_odds_plot2,
                  moose_odds_plot2,
                  bbear_odds_plot2)

# Remove the x-axis from each plot using purrr::map
lf_plots_no_xaxis <- lf_plots %>% 
  
  map( ~ .x 
       + theme(axis.title.x = element_blank()))


lf_plot <- ggarrange(plotlist = lf_plots_no_xaxis,
          ncol = 2,
          nrow = 2)

lf_plot <-  annotate_figure(lf_plot,
                             bottom = text_grob('Odds ratio'))
...
```{r}
ggsave('figures/presentation_figures/lf_odds_plot.jpg',
       lf_plot,
       width = 12,
       height = 10,
       units = 'in',
...
## Polygonal features combined plots

Same thing as with the LF plots, I want to regroup the species with polygonal features for a presentation and add color boxes to highlight the odds ratios to look at 
...
# List of plots
pol_plots <- list (lynx_odds_plot3,
                   coyote_odds_plot3,
                  hare_odds_plot3)

# Remove the x-axis from each plot using purrr::map
pol_plots_no_xaxis <- pol_plots %>% 
  
  map( ~ .x 
       + theme(axis.title.x = element_blank()))


pol_plot <- ggarrange(plotlist = pol_plots_no_xaxis,
          ncol = 2,
          nrow = 2)

pol_plot <-  annotate_figure(pol_plot,
                             bottom = text_grob('Odds ratio'))
...
```{r}
ggsave('figures/presentation_figures/pol_odds_plot.jpg',
       pol_plot,
       width = 12,
       height = 10,
       units = 'in',
